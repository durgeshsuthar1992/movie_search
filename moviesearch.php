...
if(isset($_GET['q']))
{ $moviename=$_GET['q'];
  $results=json_decode(file_get_contents("http://www.imdbapi.com/?t=".$moviename),true);
  echo "<h1>You have searched for-".$moviename."</h1>";
  ...
  mysql_query("INSERT INTO movie VALUES ('".$results["Title"]."','".$results["Director"]."','".$results["Writer"]."','".$results["Year"]."','".$results["Rated"]."','".$results["released"]."','".$results["Genre"]."','".$results["Actors"]."','".$results["Plot"]."','".$results["Runtime"]."','".$results["Rating"]."','".$results["ID"]."')",$sql);
  mysql_close($sql);
 }
 else 
